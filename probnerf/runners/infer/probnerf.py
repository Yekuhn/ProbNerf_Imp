...
from probnerf.datasets.cam_utils import build_camera_principle, build_camera_standard, surrounding_views_linspace, create_intrinsics
from probnerf.utils.logging import configure_logger
from probnerf.runners import REGISTRY_RUNNERS
from probnerf.utils.video import images_to_video
from probnerf.utils.hf_hub import wrap_model_hub
...
@REGISTRY_RUNNERS.register('infer.probnerf')
class ProbNeRFInferrer(Inferrer):

    EXP_TYPE: str = 'probnerf'
    ...
    def _build_model(self, cfg):
        from probnerf.models import model_dict
    ...
    def infer_planes(self, image: torch.Tensor, source_cam_dist: float):
        ...
        planes, mu, log_var = self.model.forward_planes(image, source_camera)
        assert N == planes.shape[0]
        return planes, mu, log_var
    ...
    def infer_single(self, image_path: str, source_cam_dist: float, export_video: bool, export_mesh: bool, dump_video_path: str, dump_mesh_path: str):
        ...
        with torch.no_grad():
            planes, mu, log_var = self.infer_planes(image, source_cam_dist=source_cam_dist)
            results = {}
            
            # Set up HMC
            hmc_kernel = HMC(self.model, step_size=0.1, num_steps=10)
            mcmc = MCMC(hmc_kernel, num_samples=10, warmup_steps=5)
            # Set up temperature scheduling
            for i in range(mcmc.num_samples):
                current_temperature = temperature_annealing(i, mcmc.num_samples)
                mcmc.kernel.step_size = 0.1 * current_temperature  # Adjust step size based on temperature
                mcmc.run(image)

            samples = mcmc.get_samples()
        
            # Use the last sample to render and export results
            last_sample = samples[-1]
            planes = last_sample['planes']
