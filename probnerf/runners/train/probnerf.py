...
from probnerf.utils.profiler import DummyProfiler
from probnerf.runners import REGISTRY_RUNNERS


logger = get_logger(__name__)


@REGISTRY_RUNNERS.register('train.probnerf')
class ProbNeRFTrainer(Trainer):
    def __init__(self):
        super().__init__()
        ...
        self.pixel_loss_fn, self.perceptual_loss_fn, self.tv_loss_fn, self.probnerf_loss = self._build_loss_fn(self.cfg)

    def _build_model(self, cfg):
        assert cfg.experiment.type == 'probnerf', \
            f"Config type {cfg.experiment.type} does not match with runner {self.__class__.__name__}"
        from probnerf.models import ModelProbNeRF
        model = ModelProbNeRF(**cfg.model)
        return model
    ...
    def _build_scheduler(self, optimizer, cfg):
        ...
        if cfg.train.scheduler.type == 'cosine':
            from probnerf.utils.scheduler import CosineWarmupScheduler
    ...
    def _build_dataloader(self, cfg):
        # dataset class
        from probnerf.datasets import MixerDataset
    ...
    def _build_loss_fn(self, cfg):
        from probnerf.losses import PixelLoss, LPIPSLoss, TVLoss, ELBOLoss
        pixel_loss_fn = PixelLoss()
        with self.accelerator.main_process_first():
            perceptual_loss_fn = LPIPSLoss(device=self.device, prefech=True)
        tv_loss_fn = TVLoss()
        probnerf_loss_fn = ELBOLoss()
        return pixel_loss_fn, perceptual_loss_fn, tv_loss_fn, probnerf_loss_fn
    ...
    def forward_loss_local_step(self, data):
        ...
        if self.cfg.train.loss.probnerf_weight > 0:
            loss_probnerf = self.probnerf_loss_fn(outputs['images_rgb'], data['targets'], outputs['mu'], outputs['log_var'])
            loss = self.cfg.train.loss.probnerf_weight * loss_probnerf

        return outputs, loss, loss_pixel, loss_perceptual, loss_tv, loss_probnerf
    ...
    def train_epoch(self, pbar: tqdm, loader: torch.utils.data.DataLoader, profiler: torch.profiler.profile):
        ...
        for data in loader:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                # forward to loss
                outs, loss, loss_pixel, loss_perceptual, loss_tv, loss_probnerf = self.forward_loss_local_step(data)
                ...
                local_step_losses.append(torch.stack([
                    ...
                    for _loss in [loss, loss_pixel, loss_perceptual, loss_tv, loss_probnerf]
                ]))
            ...
            if self.accelerator.sync_gradients:
                ...
                loss, loss_pixel, loss_perceptual, loss_tv, loss_probnerf = global_step_loss.unbind()
                loss_kwargs = {
                    'loss': loss.item(),
                    'loss_pixel': loss_pixel.item(),
                    'loss_perceptual': loss_perceptual.item(),
                    'loss_tv': loss_tv.item(),
                    'loss_probnerf': loss_probnerf.item(),
                }
        ...
        epoch_loss, epoch_loss_pixel, epoch_loss_perceptual, epoch_loss_tv, epoch_loss_probnerf = epoch_losses.unbind()
        epoch_loss_dict = {
            'loss': epoch_loss.item(),
            'loss_pixel': epoch_loss_pixel.item(),
            'loss_perceptual': epoch_loss_perceptual.item(),
            'loss_tv': epoch_loss_tv.item(),
            'loss_probnerf': epoch_loss_probnerf.item(),
        }
        self.log_scalar_kwargs(
    ...
    def evaluate(self, epoch: int = None):
        ...
        total_loss, total_loss_pixel, total_loss_perceptual, total_loss_tv, total_loss_probnerf = total_losses.unbind()
        total_loss_dict = {
            'loss': total_loss.item(),
            'loss_pixel': total_loss_pixel.item(),
            'loss_perceptual': total_loss_perceptual.item(),
            'loss_tv': total_loss_tv.item(),
            'loss_probnerf':total_loss_probnerf.item(),
        }
