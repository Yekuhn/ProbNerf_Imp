...
import numpy as np
import gradio as gr
from probnerf.models.hypernetwork import generate_parameters
...
def init_preprocessor():
    from probnerf.utils.preprocess import Preprocessor
    global preprocessor
...
def demo_probnerf(infer_impl):

    def core_fn(image: str, source_cam_dist: float, working_dir):
        # Generate dynamic parameters for NeRF from HyperNetwork
        z = torch.randn((1, 128))  # Example latent vector
        nerf_params = generate_parameters(z)
    ...
    _TITLE = '''ProbNerf: LRM-based Probabilistic NeRF Models'''

    _DESCRIPTION = '''
        <strong>Disclaimer:</strong> This demo is based on `openlrm-mix-base-1.1` model with 288x288 rendering resolution here for a quick demonstration.
    '''
    ...
    with gr.Blocks(analytics_enabled=False) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(variant='panel', scale=1):
                with gr.Tabs(elem_id="probnerf_render_video"):
        ...
        with gr.Row():
            with gr.Column(variant='panel', scale=1):
                with gr.Tabs(elem_id="probnerf_attrs"):
                    with gr.TabItem('Settings'):
                        with gr.Column(variant='panel'):
                            ...
                            submit = gr.Button('Generate', elem_id="probnerf_generate", variant='primary')
...
def launch_gradio_app():
    ...
    from probnerf.runners.infer.base_inferrer import Inferrer
    InferrerClass : Inferrer = REGISTRY_RUNNERS[os.getenv("APP_TYPE")]
    with InferrerClass() as inferrer:
        init_preprocessor()
        if not bool(os.getenv('SPACE_ID')):
            from probnerf.utils.proxy import no_proxy
            demo = no_proxy(demo_probnerf)
        else:
            demo = demo_probnerf
